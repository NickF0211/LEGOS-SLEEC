...
def parse_element(node, Action_Mapping, head=None, measure=None):
    ...
    if isXinstance(node, "Rule"):
        ...
        res = parse_extended_response(node, Action_Mapping, head, measure)
...
def parse_violation(node, Action_Mapping, current, violation):
    assert isXinstance(node, "Occ")
    negation = node.neg
    event = parse_element(node.event, Action_Mapping, current, current)
    if node.limit:
        start, end = parse_element(node.limit, Action_Mapping, current, current)
    else:
        start = end = 0
...
def parse_extended_response(node, Action_Mapping, head, measure):
    first = parse_element(node.head, Action_Mapping, head, measure)
    if node.next is not None:
        second = parse_element(node.next, Action_Mapping, head, measure)
...
def parse_response(node, Action_Mapping, head, measure):
    ...
    if node.alternative:
        if timed_occ(node.occ):
            alt = exist(Action_Mapping["Measure"], lambda violation_m, node=node: AND(
                parse_violation(node.occ, Action_Mapping, measure, violation_m),
    ...
    if node.nd is not None:
        main_action = OR(main_action, parse_element(node.nd, Action_Mapping, head, measure))
...
def check_concerns(model, rules, concerns, Action_Mapping, Actions, model_str="", to_print=True, multi_entry=False):
...
def check_purposes(model, purposes, rules, Action_Mapping, Actions, model_str="", check_proof=False, to_print=True,
                   multi_entry=False):
    Measure = Action_Mapping["Measure"]
    measure_inv = forall([Measure, Measure], lambda m1, m2: Implication(EQ(m1.time, m2.time), EQ(m1, m2)))
    output = ""
    ...
    for i in range(len(purposes)):
        ...
        purpose = purposes[i]
        res = check_property_refining(purpose.get_concern(), set(), [r.get_rule() for r in rules] + [measure_inv],
                                      Actions, [], True,
                                      min_solution=False,
                                      final_min_solution=True, restart=False, boundary_case=False,
                                      universal_blocking=False, vol_bound=VOL_BOUND,
...
def check_red(model, rules, Action_Mapping, Actions, model_str="", check_proof=False, to_print=True, multi_entry=False):
    Measure = Action_Mapping["Measure"]
    measure_inv = forall([Measure, Measure], lambda m1, m2: Implication(EQ(m1.time, m2.time), EQ(m1, m2)))
...
def parse_sleec(model_file, read_file=True):
    ...
    return model, rules, concerns, purposes, Action_Mapping, Actions
...
def check_input_red(model_str, multi_entry=False):
    model, rules, concerns, purposes, Action_Mapping, Actions = parse_sleec(model_str, read_file=False)
...
def check_input_purpose(model_str, multi_entry=False):
    model, rules, concerns, purposes, Action_Mapping, Actions = parse_sleec(model_str, read_file=False)
    res = check_purposes(model, purposes, rules, Action_Mapping, Actions, check_proof=True, model_str=model_str,
                         multi_entry=multi_entry)
    # reset
    scalar_mask.clear()
    scalar_type.clear()
